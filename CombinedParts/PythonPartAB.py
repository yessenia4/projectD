...
class SearchEngine:
    ...
    def getHitList(self,query):
        ...
        queryTree = Q.p(query)
        #Q.print_tree_indented(queryTree,0)
        hitList = Processor.processTree(queryTree,self.termList,self.docList)
        #rank results
        rankedList = self.sim(hitList,self.termList,self.docList,self.getQTerms(query))
...
while True:
    ...
    queryTree = Q.p(query)
    #Q.print_tree_indented(queryTree,0)
    hitList = Processor.processTree(queryTree,result[0],result[1])
    #rank results
    rankedList = sim(hitList,result[0],result[1],getQTerms(query))

    print("Hits: " + repr(len(rankedList)))
    count = 1
    #looks through the results....
    for fileID in rankedList:
        if( count <= 3):
            d = result[1].get(fileID[0])
            if (d != None):
                temp = d['URL']
                print(temp)
            #print("\n")
